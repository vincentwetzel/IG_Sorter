...
void CleanupScreen::setDirectories(const QStringList& dirs) {
    ...
    for (auto* bar : std::as_const(m_progressBars)) {
        delete bar;
    }
    for (auto* label : std::as_const(m_progressLabels)) {
        delete label;
    }
    for (auto* label : std::as_const(m_pathLabels)) {
        delete label;
    }
    ...
    m_resolutionWidget->setUpdatesEnabled(false);
    ...
    m_resolutionWidget->setUpdatesEnabled(true);
...
void CleanupScreen::showUnresolvedIssues(const QStringList& unknownNames, DatabaseManager* db) {
    ...
    m_resolutionRows.reserve(unknownNames.size());
